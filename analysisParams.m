...
RLSplotOrNot = 'yes'; %timecourses of RLS data
...
parPlotOrNot = 'yes'; %individual transition plot
...
if group == 0
    parNums = [119];%[105:140];%[138]; % ex/ Stratus100's parNum is 100 %Note, CER = 137; Range = [105:140]
    group = 'stratus'; % 'stratus' or 'cumulus'
    % exclude participants
    load('pre-processing/stratExcluded.mat')
    parNums = setdiff(parNums,stratExcluded);
else
    parNums = [1:5]; % ex/ Stratus100's parNum is 100; Range = [1:28]
...
% Select runs
runTypes = {'dartRival'}; % {'dartRival', 'dartSim', 'marzRival'}
runIndices = [1]; % generally 1:3
...
elseif strcmp(paramsFlag, 'parPlot')
    ...
    dom1Min = 0.5; %0.5 %1.25; %cutoff for shortest d ominant state to be included
    dom1Max = inf;
    dom2Min = 0.5; %0.5 %cutoff for shortest dominant state to be included
    dom2Max = inf;
    mixedMin = 0; %.2 %1;
    mixedMax = inf; %.75
    noMixed = 1; % if this is 1, do not include transitions with a mixed component
...
elseif strcmp(paramsFlag, 'ampClust')
    ...
    latency = round(0*sampRate); % predicted latency in data points (sampRate = how many points / second, so 0.8*sampRate = 800 ms)
