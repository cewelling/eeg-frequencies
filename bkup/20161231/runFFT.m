...
%     end
    ...
    if strcmp(FFTplotOrNot, 'yes')
        ...
        %                     figure;
        %                     plot(freqs.freq, powspctrm(iTrial,badElecs(iElec).num),:);
        %                     box off
        %                     title(['FFT Results: ' parName ', ' runName ', Electrode' num2str(badElecs{iTrial}(i).num) ...
        %                         ', Kurtosis: ' num2str(badElecs{iTrial}(i).kur)])
        %                 end
        %             end
        %         end
