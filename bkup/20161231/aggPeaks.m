...
for iPeakFreq = 1:length(peakIndices) % for peaks in both stim freqs
    ...
    for iPeak = 1:length(trials)
        ...
        elseif (peakIndex + halfLength) > length(RLSf1) - (timeWindow / 1000 * 512); % remember end is padded
            epochRLSf1 = RLSf1(peakIndex - halfLength : length(RLSf1) - (timeWindow / 1000 * 512));
            epochRLSf2 = RLSf2(peakIndex - halfLength : length(RLSf2) - (timeWindow / 1000 * 512));
            padSize = fullLength - length(epochRLSf1);
