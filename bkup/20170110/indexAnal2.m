...
% save full analysis matrix
if strcmp(analType, 'darts')
    save('indices/darts_analMatrix2', 'fullAnalMatrix');
else
    save('indices/marz_analMatrix2', 'fullAnalMatrix');
end
...
for i = 1:5 %size(fullAnalMatrix, 2)
    for j = 6:12 %i+1:size(fullAnalMatrix,2)
            ...
            if abs(P(i,j)) < 0.1
                ...
                validi = ~isnan(fullAnalMatrix(:,i));
                validj = ~isnan(fullAnalMatrix(:,j));
                validBoth = validi & validj;
                p = polyfit(fullAnalMatrix(validBoth,i),fullAnalMatrix(validBoth,j),1);   % p returns 2 coefficients fitting r = a_1 * x + a_2
                line = p(1) .* fullAnalMatrix(validBoth,i) + p(2); % compute a new vector r that has matching datapoints in x     
                plot(fullAnalMatrix(validBoth,i), line, '-'); %axis([-1 14 0 2])
                ...
                % save figures
                if strcmp(analType, 'darts')
                    figName = [corrPlotDir label '_' C{i} 'vs' C{j} '_DARTS.jpg'];
                else
                    figName = [corrPlotDir label '_' C{i} 'vs' C{j} '_MARZ.jpg'];
                end
