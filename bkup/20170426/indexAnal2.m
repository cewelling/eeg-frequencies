...
% Column 35: Hertz
% Column 36: HertzMedian
% Column 37: HertzTrans
% Column 38: HertzTransMedian
% Column 39: HertzTransNoRevsMedian
% Column 40: HertzTransNoRevsMean
...
% more behavioral indices
C{35} = 'Hertz';
C{36} = 'HertzMedian';
C{37} = 'HertzTrans';
C{38} = 'HertzTransMedian';
C{39} = 'HertzTransNoRevsMedian';
C{40} = 'HertzTransNoRevsMean';
...
for iGroup = 1:length(groupParNums)
    for parNum = groupParNums{iGroup}
        ...
        % Hertz
        if exist([bResultsDir parName '_' analType '_Hertz.txt'])
            analRow(35) = load([bResultsDir parName '_' analType '_Hertz.txt']);
        end
        ...
        % HertzMedian
        if exist([bResultsDir parName '_' analType '_HertzMedian.txt'])
            analRow(36) = load([bResultsDir parName '_' analType '_HertzMedian.txt']);
        end
        
        % HertzTrans
        if exist([bResultsDir parName '_' analType '_HertzTrans.txt'])
            analRow(37) = load([bResultsDir parName '_' analType '_HertzTrans.txt']);
        end
        
        % HertzTransMedian
        if exist([bResultsDir parName '_' analType '_HertzTransMedian.txt'])
            analRow(38) = load([bResultsDir parName '_' analType '_HertzTransMedian.txt']);
        end
        
         % HertzTransNoRevsMedian
        if exist([bResultsDir parName '_' analType '_HertzTransNoRevsMedian.txt'])
            analRow(39) = load([bResultsDir parName '_' analType '_HertzTransNoRevsMedian.txt']);
        end
        
         % HertzTransNoRevsMean
        if exist([bResultsDir parName '_' analType '_HertzTransNoRevsMean.txt'])
            analRow(40) = load([bResultsDir parName '_' analType '_HertzTransNoRevsMean.txt']);
        end
...
%         end
    ...
    [R, P] = corr(analMatrix{iGroup},analMatrix{iGroup}, 'type','spearman', 'rows', 'pairwise');
