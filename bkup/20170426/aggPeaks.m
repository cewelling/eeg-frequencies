...
if altPeak && ~exist(['altpLists/' parName '_' runName '.mat'], 'file') || ...
        ~altPeak && ~exist(['pLists/' parName '_' runName '.mat'], 'file')
...
% Epoch structure reminder: 1: Trial; 2: Start; 3: End; 4: button pressed; 5: frequency / mixed; 

% if strcmp(runName, 'dartRival3')
%     disp('hi')
% end
...
if altPeak
    save(['altpLists/' parName '_' runName], 'peakIndices', 'peakTrials', 'peakLengths')
else
    save(['pLists/' parName '_' runName], 'peakIndices', 'peakTrials', 'peakLengths')
end

elseif altPeak && exist(['altpLists/' parName '_' runName '.mat'], 'file') 
    load(['altpLists/' parName '_' runName '.mat'])
else
    load(['pLists/' parName '_' runName '.mat'])
end
...
for iTrial = 1:size(rls_data(1).amp, 2)
    for iFreq = 1:length(rls_data);
        ...
        elseif strcmp(normType, 'noStimBase')
            normRLSamp = RLSamp - baselines(iFreq);
        elseif strcmp(normType, 'freqWin')
...
% save peaks
if altPeak
    save(['altpListsRLS/' parName '_' runName '_' normType], 'peaks');
else
    if analFreqs == stimFreqs
        save(['pListsRLS/' parName '_' runName '_' normType], 'peaks');
    elseif analFreqs == harFreqs
        save(['pListsRLS/' parName '_' runName '_' normType '_harmonics'], 'peaks');
    end
end
