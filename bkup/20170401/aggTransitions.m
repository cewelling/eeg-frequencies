...
if ~exist(['tLists_CERW/' parName '_' runName '.mat'], 'file')
%if ~exist(['tLists/' parName '_' runName '.mat'], 'file')
    ...
    load(['tLists_CERW/' parName '_' runName '.mat'])
...
for iCase = 1:size(caseTrials,2) %for each type of transition
    ...
    for iEpoch = 1:length(thisCaseTrials)
        ...
        % epoch is cut off by beginning, do not include it % pad beginning with NaNs
...
%             epochRLSf2 = [nan(1,padSize), epochRLSf2];
            ...
            % epoch is cut off by end, do not include it %pad end with NaNs
...
%             epochRLSf2 = [epochRLSf2,nan(1,padSize)];
    ...
    elseif isequal(analFreqs, harFreqs)
        save(['tListsRLS/' parName '_' runName '_' normType '_harmonics'], 'transitions');
    end
