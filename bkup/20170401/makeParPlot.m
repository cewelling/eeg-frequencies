...
for i = 1:length(runList)
    ...
    if exist(['tListsRLS/' parName '_' runName '_' normType '.mat'], 'file') && isequal(analFreqs,stimFreqs)
        load(['tListsRLS/' parName '_' runName '_' normType '.mat'])
    elseif exist(['tListsRLS/' parName '_' runName '_' normType '_harmonics.mat'], 'file') && isequal(analFreqs,harFreqs)
        load(['tListsRLS/' parName '_' runName '_' normType '_harmonics.mat'])
    else
...
%     end
    ...
    hold on
    mseb([(-transHalf):(1/512):transHalf],[s2dTrace; d2sTrace],[s2dError; d2sError],[],1);
    ylim([-1.5 1.5])
    vline(0,'k','transition')
    xlabel('Time from button press (s)');
    ylabel('Amplitude');
