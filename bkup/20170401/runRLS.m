 ...
 if strcmp(RLSplotOrNot, 'yes')
     ...
     for iTrial = 1:size(all_data.trial, 2)
         ...
         for iEpoch = 1:size(trialEpochs,1)
             ...
             eStart = trialEpochs(iEpoch,2);
             eEnd = trialEpochs(iEpoch,3);
             horiz = [eStart eEnd];
             vert = [shadeLim shadeLim];
             
             % account for latency
             if eStart > latency
                 eStart_lat = eStart - latency;
             else
                 eStart_lat = 1;
             end
             ...
             if trialEpochs(iEpoch,5) > 0.5
                 ...
                 lOppAmps = [lOppAmps lowBand(eStart_lat:eEnd - latency)];
                 hCorrAmps = [hCorrAmps lowBand(eStart_lat:eEnd - latency)];
             elseif trialEpochs(iEpoch,5) < -0.5
                 area(horiz,vert,'facecolor','b','facealpha',0.15,'edgealpha',0.1);
                 lCorrAmps = [lCorrAmps lowBand(eStart_lat:eEnd - latency)];
                 hOppAmps = [hOppAmps lowBand(eStart_lat:eEnd - latency)];
             else
                 area(horiz,vert,'facecolor','y','facealpha',0.15,'edgealpha',0.1);
                 lMixedAmps = [lMixedAmps lowBand(eStart_lat:eEnd - latency)];
                 hMixedAmps = [hMixedAmps lowBand(eStart_lat:eEnd - latency)];
             end
