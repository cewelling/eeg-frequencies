% SVMcrossVal: Cross-validation using 4 controls excluded for IQ matching. Trains on 3 participants,
% tests on 1 left out participant. Loops through all 4 testing participant
% possibilities and averages accuracies.
%
...
if ~isempty(strfind(testSet,'ransitions'))
    error('We optimize on segments, not timecourses. Change testSet in svmParams.m before running this script.')
end
if ~strcmp(testSet, 'identical')
    error('Why would you run an across participant cross-validation with different testing and training sets? Change testSet in svmParams.m.')
end
...
for parNum = parNums
    ...
    elseif strcmp(testSet, 'subset')
        ...
        if testExist
            load(['ML/parTestSets/' parName '_' testSet '_' num2str(numStates) 'states.mat']);
        end
    end
...
for iModel = 1:length(models)
    ...
    for iKs = 1:length(modSigmas)
        ...
        for iC = 1:length(Cs);
            ...
            for iTestPar = 1:length(parNums)
                
                fprintf(repmat('\b',1, Msglength));
                Msg = sprintf('Running participant %d of %d\n', iTestPar, length(parNums));
                fprintf(Msg);
                Msglength = numel(Msg);
                
                % participant-specific set-up
                testParNum = parNums(iTestPar);
                numFormat = '%02d';
                ...
                if withMixed
                    ...
                    SVMModel = fitcsvm(trData, trLabels, 'KernelFunction', 'linear', 'BoxConstraint', cVal, 'Standardize', true, 'ClassNames', {'lo','hi'});
                end
