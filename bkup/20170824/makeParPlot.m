function [ tLists, tLabels, pLists, pLabels, ntLists, l2hAvgDelay, h2lAvgDelay, avgDelay] = makeParPlot( parName, iPar, date, paramsFlag)
...
for iElec = [1:length(analElecs) 0] % 0 for the average of the selected electrodes
    ...
    % space to store delays between sim schedule changes and button presses
    l2hParDelays = [];
    h2lParDelays = [];
    ...
    for iRunType = 1:length(runTypes)
        ...
        for runIndex = runIndices
            ...
            elseif strfind(runName, 'dartSim')
                ...
                % sim delays (when aligning to button presses)
                if ~useSimSchedule && iElec == 0
                    l2hParDelays = [l2hParDelays; l2hPressDelays(tKeep{1})];
                    h2lParDelays = [h2lParDelays; h2lPressDelays(tKeep{2})];
                end
...
%                 h2hMarzRival_run = structSubset(h2hMarzRival, runStart);
                ...
                if strcmp(cRunType, 'dartSim') && useSimSchedule
                    save(['transitions/transitionsByRun_simSched/' parName '_' runName '_' normType '_' analFreqLabel '_' num2str(thisElec)], 'run_tLists');
                else
...
%% Store average delay between sim schedule change and button press

if ~useSimSchedule && strfind(runName, 'dartSim')
    % handle missed button presses
    numMissed = length(find(abs(l2hParDelays) >= 1)) + length(find(abs(h2lParDelays) >= 1));
    if numMissed > 12
        error('> 12 missed button presses. Is it a coding error, or is this participant unreliable?');
    end
    l2hParDelays = l2hParDelays(abs(l2hParDelays) < 1);
    h2lParDelays = h2lParDelays(abs(h2lParDelays) < 1);
    
    % average delay between sim schedule change and button press
    l2hAvgDelay = nanmean(l2hParDelays);
    h2lAvgDelay = nanmean(h2lParDelays);
    avgDelay = nanmean([l2hParDelays; h2lParDelays]);
end
...
if useSimSchedule 
    ...
    save(['transitions/finalTransitions/' parName '_' normType '_' analFreqLabel '_' electrodeSet.name], 'tLists', 'l2hAvgDelay', 'h2lAvgDelay', 'avgDelay');
end
...
if strcmp(parPlotOrNot, 'yes')
    ...
    ylim([-0.8 0.8])
    if ~useSimSchedule
        vline(-avgDelay, 'b', 'stim change');
        vline(0, 'k', 'button press');
    else
        vline(0,'k','stim change')
    end
    xlabel('Time from button press (s)');
    ylabel('Amplitude');
    legend('Frequency 1', 'Frequency 2');
