...
if ~isempty(strfind(runName, 'Sim')) && useSimSchedule
    ...
    if ~isempty(strfind(runName, 'Sim')) % get sim schedule for simulation runs
        ...
        l2hSchedTrials = l2h{2};
        h2lSimSwitches = h2l{1};
        h2lSchedTrials = h2l{2};
...
% store delay between stimulus switch and button press for each transition 
l2hPressDelays = [];
h2lPressDelays = [];

for iCase = 1:size(caseTrials,2) %for each type of transition
    
    %list of trials containing this type of transition
    thisCaseTrials = caseTrials(~isnan(caseTrials(:,iCase)),iCase); 
    ...
    for iEpoch = 1:length(thisCaseTrials)
        ...
        % Find time of simulation switch relative to button press
        if ~isempty(strfind(runName, 'Sim')) && ~useSimSchedule && iCase <= 2 % l2h or h2l for a sim run aligned to button press
            if iCase == 1 % l2h
                simSwitches = l2hSimSwitches;
                simSwitchTrials = l2hSchedTrials;
            elseif iCase == 2 % h2l
                simSwitches = h2lSimSwitches;
                simSwitchTrials = h2lSchedTrials;
            end
            
            % get stimulus switches for this trial
            trialSwitches = simSwitches(simSwitchTrials == thisCaseTrials(iEpoch));
            
            % calculate delay (time between button press and closest stimulus switch)
            % Taking the min works because all sim schedule epochs are at least one second--longer than any latency
            [~,thisDelayI] = nanmin(abs(transitionTime - trialSwitches)); % in seconds
            thisDelay = transitionTime - trialSwitches(thisDelayI);
            
            if iCase == 1 % l2h
                l2hPressDelays = [l2hPressDelays; thisDelay];
            elseif iCase == 2 % h2l
                h2lPressDelays = [h2lPressDelays; thisDelay];
            end
        end
