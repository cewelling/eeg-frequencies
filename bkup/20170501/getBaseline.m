...
%  end
 ...
 for iTrial = 1:size(all_data.trial, 2); % # trials recorded in EEG (should = numTrials)
     ...
     for iStimFreq = 1:length(stimFreqs) % size([stimFreqs imFreqs])
         stimFreq = stimFreqs(iStimFreq);
         
         % determine frequencies for baseline calculation
         rHalf = 2.125; %1.5; % range: stimFreq +/- freqRange
         numFreqs = 5; %10;
...
%              ylim([0 6])
             ...
             rls_data{thisTrial}{iStimFreq}(iFreq,:) = envelope;
         end
     end
 end
