...
for iElectrodes = 1:size(electrodeSets, 2)
    ...
    for iTrial = 0:size(freqs.powspctrm,1) 
        ...
        for i = 1:length(maxSNRelecs)
            ...
            lFreqSNRs(1,i) = maxSNRs(1,i);
            hFreqSNRs(1,i) = maxSNRs(1,i);
            
            % SNR value in second row
            maxSNRs(2,i) = sortedSNRs(i);
            lFreqSNRs(2,i) = lowSNRs(i);
            hFreqSNRs(2,i) = highSNRs(i);
        ...
        % Account for mistakes
         ...
         if iTrial == 0
             ...
             save(['highSNRelecs/' parName '_' runName '_trial' num2str(thisTrial) '_'  num2str(numElecs) 'elecs'], 'maxSNRs', 'lFreqSNRs', 'hFreqSNRs');
         end
        ...
        if strcmp(FFTplotOrNot, 'yes')
            ...
            if iTrial == 0
                ...
                title(['FFT Results: ' parName ', ' runName ', trial ' num2str(thisTrial) ', ' electrodeSets(iElectrodes).name ])
                figName = [fftSpectPlotDir parName '_' runName '_trial' num2str(thisTrial) '_FFT_' electrodeSets(iElectrodes).name '.jpg'];
            end
