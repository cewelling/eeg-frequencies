...
%% Find transitions -- include those that go through mixed state
if allTrans == 1
    [lowToHigh, highToLow, l2hTrials, h2lTrials, l2hMids, h2lMids] = findTransitions(epochs);
    
    caseTimes(1:length(lowToHigh),1) = lowToHigh;
    caseTrials(1:length(l2hTrials),1) = l2hTrials;
    caseTimes(1:length(highToLow),2) = highToLow;
    caseTrials(1:length(h2lTrials),2) = h2lTrials;
    ...
    if isempty(rls_data) %SNR too low in this run, return empty transitions array
        fprintf('SNR of %s %s too low to create time series\n', parName, runName);
        for iCase = 1:size(caseTrials, 2)
            transitions(iCase).type = caseType(iCase);
            transitions(iCase).f1 = [];
            transitions(iCase).f2 = [];
        end
        return;
    end
    ...
    %if exist([blineDir parName '_' runName '_' blineLoc], 'file')
