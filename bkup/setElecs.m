% Old script, previously used to select best x electrodes out of y given
% electrodes based on SNR

function [elecs] = setElecs( parName )
...
SNRfiles = dir(['pre-processing/highSNRelecs/' parName '_dart*_' num2str(numElecs) 'elecs.mat']);
if ~isempty(SNRfiles)
    
    elecArray = {}; % store high SNR electrodes from each run
    for thisRun = 1:length(SNRfiles)
        load(['pre-processing/highSNRelecs/' SNRfiles(thisRun).name])
...
%         end
    ...
    save(['pre-processing/highSNRelecs/' parName '_darts_' num2str(commonElecs) 'of' num2str(numElecs) 'elecs'], 'elecs');
end
