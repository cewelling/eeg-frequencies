...
% Do amplitudes cluster according to perceptual report? (Dominant vs. Mixed
% vs. Suppressed?)

% Load parameters
analysisParams

%% Important parameters for this analysis

% Predicted latency
latency = round(.8*sampRate); % in data points
...
for iRunType = 1 % dartRival only
    ...
    for runIndex = runIndices
        ...
        for iTrial = 1:size(rls_data(1).amp, 2)
            ...
            %% Collect amplitudes from dominant, suppressed, and mixed epochs
