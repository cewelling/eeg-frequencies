function [ all_data ] = ftConfigParams( parName, runName, EEGfile, numTrials, customDur, date, event)
...
if nargin == 6
    event = 'trial';
elseif nargin < 6
    error('Not enough arguments for ftConfigParams');
end
...
cfg_trldef.trialdef.prestim = discard_start; %discard_start;
...
refElecs = refElecs(goodIndices); 
 ...
 cfg_preproc.refmethod = 'avg'; % median %cer 11/16
...
cfg_preproc.bpfreq = [2 70]; %[4 10]; %[2 70]; % bandpass frequencies 
