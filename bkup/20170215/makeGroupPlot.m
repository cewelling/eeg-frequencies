...
for iGroup = 1:length(groupParNums)
    ...
    for parNum = groupParNums{iGroup};
        ...
        for tType = 5:6 % 5: sim low to high; 6: sim high to low
            ...
            % average transitions
            f1trace = nanmean(tLists{tType}.f1);
            f2trace = nanmean(tLists{tType}.f2);
...
%         end
        ...
        save(['normTransitions/' parName '_' normType], 'ntLists');
...
%         f2hsimAmp = nanmean(f2highMaxes) - nanmean(f2highMins);
        ...
        save(['normPeaks/' parName '_' normType], 'npLists');
