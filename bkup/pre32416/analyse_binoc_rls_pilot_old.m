...
elseif exist('/Users/jacksonclee/Dropbox (MIT)/Projects/EEG_Frequencies/data', 'dir')
    file_directory = '/Users/jacksonclee/Dropbox (MIT)/Projects/EEG_Frequencies/data';
elseif exist('/Users/robertsonce/Dropbox (MIT)/Projects/EEG_Frequencies/data', 'dir')
    file_directory = '/Users/robertsonce/Dropbox (MIT)/Projects/EEG_Frequencies/data';
else
...
fileNames = dir('../data/caroline*3_14*');
fileNames = {fileNames.name};
filenames{1}(1).name = char(fileNames(analRunNum));
legend = importdata('pilotlegends/pilot-day3-caroline.txt');
...
cfg_preproc.refchannel = electrodes; %{'01', 'Oz', 'Iz', 'POz'}; %%electrodes
...
cfg_tfr = [];
cfg_tfr.channel = 'all';
cfg_tfr.foi = 0:0.25:60; %frequency of interest
...
slidingWindowWidth = 1.2*ones(size(cfg_tfr.foi)); %window specific for each frequency

cfg_tfr.method = 'mtmconvol';
cfg_tfr.taper = 'hanning'; %sliding window shape (tapered)
cfg_tfr.t_ftimwin = slidingWindowWidth;
...
yLims = [1.5 1.5; 2 2]; %trial 3
zLims = [0 6; 0 10]; % trial 3
%yLims = [.1 2; 3 1]; %trial 5
zLims = [0 6; 0 6]; %trial 5
for iTrial = 1:trialNum
    figure;
    for iFreq = 1:2
        subplot(2, 1, iFreq);
