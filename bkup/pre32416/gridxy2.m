...
%     gridxy2(get(gca,'XTick'),get(gca,'YTick'), 'Color',[1,1,1]*0.4,'Linestyle',':');
...
xx1 = repmat(x(:).',3,1) ;
yy1 = repmat([ylim1(:) ; nan],1,numel(x)) ;

% setup data for the horizontal lines
xx2 = repmat([xlim1(:) ; nan],1,numel(y)) ;
yy2 = repmat(y(:).',3,1) ;
...
if ~isempty(xx1),     
    ...
    set(hca,'ylim',ylim1,'xlim',xlim1) ; % reset the limits
