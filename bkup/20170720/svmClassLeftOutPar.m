function [accuracies, postProbs, confusionMats, mixedAccs, domAccs] = svmClassLeftOutPar(trainSet, testSet, rivLat, segPoints, domSegMid_sim, domSegMid_riv, mixSegProp, teStartPt, teEndPt, paramsFile, withMixed, numStates)
...
for iGroup = 1:length(groupParNums)
    ...
    for iTestPar = 1:length(groupParNums{iGroup})
        ...
        if withMixed
            ...
            elseif numStates == 2
                SVMModel = fitcsvm(trData, trLabels, 'KernelFunction', 'linear', 'BoxConstraint', 0.01, 'Standardize', false, 'ClassNames', {'dom','mi'});
                %SVMModel = fitcsvm(trData, trLabels, 'KernelFunction', 'rbf', 'BoxConstraint', 0.01, 'KernelScale', 1, 'Standardize', false, 'ClassNames', {'dom','mi'});
            end
        else
            SVMModel = fitcsvm(trData, trLabels, 'KernelFunction', 'linear', 'BoxConstraint', 0.01, 'Standardize', false, 'ClassNames', {'lo','hi'});
        end
