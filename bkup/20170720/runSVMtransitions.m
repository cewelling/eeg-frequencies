% Train an SVM to classify dominant state at each point in a transition
...
for iBin = 1:length(teStartPts)
    ...
    [accuracies, postProbs] = svmClassify(trainSet, testSet, rivLat, segPoints, domSegMid_sim, domSegMid_riv, mixSegProp, teStartPt, teEndPt, numStates, paramsFile);
...
save([saveDir group '_train-' trainSet '_test-' testSet '_' num2str(length(accuracies)) 'pars'], 'accuracyMat', 'postProbMat', 'teTime');
