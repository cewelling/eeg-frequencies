...
for iFreq = 1:length(analFreqs) % size([stimFreqs imFreqs])
    ...
    if exist(['pre-processing/preproc/' parName '_' runName '_freq' strrep(num2str(analFreqs(iFreq)),'.','-') '.mat'], 'file')
        ...
        try
            all_data = ftPrep(parName, runName, EEGfile, numTrials, date, analFreqs(iFreq) - 0.5, analFreqs(iFreq) + 0.5, paramsFlag, cfg_preproc);
        catch
...
%             fprintf(repmat('\b',1,msglength));
            ...
            if sum(isnan(all_data.trial{iTrial}(analElecs,:))) > 100
                envelope = nan(1,length(envelope));
            end
        ...
        % CER 2017
        if isempty(shiftedEnv) || sum((shiftedEnv)==0)>100 
            shiftedEnv = nan(1,length(shiftedEnv));
        end
