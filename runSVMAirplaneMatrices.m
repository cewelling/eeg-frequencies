...
for iLat = 1:length(rivLats)
    ...
    for iSeg = 1:length(segTime) 
        ...
        for iBin = 1:length(teStartPts)
            ...
            if withMixed
                [accuracies, ~, confusionMats] = svmClassify_withMixed(trainSet, testSet, rivLat, segPoints, domSegMid_sim, domSegMid_riv, mixSegProp, teStartPt, teEndPt, numStates);
            else
                [accuracies, ~] = svmClassify(trainSet, testSet, rivLat, segPoints, domSegMid_sim, domSegMid_riv, mixSegProp, teStartPt, teEndPt, numStates);
        ...
        fprintf('Saving accuracy timecourses for each participant\n') 
    end
end
...
fprintf('Saving airplane matrices\n');
