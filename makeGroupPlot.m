...
% makeGroupPlot() creates group transition and peak plots for
% stratus (control) and cumulus (ASC) participants.
%
% Transition plots are saved in the plots/transitions/group folder. Lists
% of transitions and peaks for each participant are saved in the
% transitions/ and peaks/ folders. In the transitions/finalTransitions/
% folder, each file loads the variable tLists, which contains a list of all
% of a participants transitions. In the transitions/finalTransitions_norm
% folder, each file loads the variable ntLists, which contains a single
% averaged, normalized transition for the participant.
%
% Called by: groupAnalysisController.m
% Dependencies: groupAnalysisParams.m, makeParPlot.m
...
for iGroup = 1:length(groupParNums)
    ...
    % space to store delays between sim stimulus change and button press
    l2hAvgDelays = [];
    h2lAvgDelays = [];
    avgDelays = [];
        ...
        if loaded
            try
                ...
                if useSimSchedule              
                    ...
                    load(['transitions/finalTransitions_simSched/' parName '_' normType '_' analFreqLabel '_' electrodeSet.name '.mat'])
                    
                    % normalized transitions
                    %load(['transitions/finalTransitions_norm/' parName '_' normType '.mat'])
                    load(['transitions/finalTransitions_norm_simSched/' parName '_' normType '_' analFreqLabel '_' electrodeSet.name '.mat'])                    
                else
                    % transitions
                    load(['transitions/finalTransitions/' parName '_' normType '_' analFreqLabel '_' electrodeSet.name '.mat'])
                    
                    % normalized transitions
                    load(['transitions/finalTransitions_norm/' parName '_' normType '_' analFreqLabel '_' electrodeSet.name '.mat'])
                end
                
            catch
                paramsFlag = 'parPlot';
                if useSimSchedule
                    input('Ensure that useSimSchedule = 1 in the parPlot section of analysisParams.m \n ... Press any key to continue.')
                else
                    input('Ensure that useSimSchedule = 0 in the parPlot section of analysisParams.m \n ... Press any key to continue.')
                end
                [ tLists, ~, pLists, ~, ntLists, l2hAvgDelay, h2lAvgDelay, avgDelay] = makeParPlot(parName, [], date, paramsFlag);
            end
        else
            paramsFlag = 'parPlot';
            [ tLists, ~, pLists, ~, ntLists, l2hAvgDelay, h2lAvgDelay, avgDelay] = makeParPlot(parName, [], date, paramsFlag);
...
%                 end
%                 
%                 % Plot a participant plot with peaks marked
...
%                     normedF1 = (currTf1 - min(currTf1)) / f1simAmp;
%                     normedF2 = (currTf2 - min(currTf2)) / f2simAmp;
...
%        save(['transitions/finalTransitions_norm/' parName '_' normType], 'ntLists');
        ...
        %% Collect sim stimulus change to button press delays
        if ~useSimSchedule
            l2hAvgDelays = [l2hAvgDelays l2hAvgDelay];
            h2lAvgDelays = [h2lAvgDelays h2lAvgDelay];
            avgDelays = [avgDelays avgDelay];
        end
    ...
    %% clean edges of normalized transitions, which have been realigned to transition crosses
    %(if one transition cuts off, cut all of them off)
    
    if alignToCross
        
        % find highest nan in first half
        [~, col] = find(isnan(groupTLists{15}.f1(:,1:floor((length(alignedS2D))/2))));
        groupTLists{15}.f1(:,1:max(col)) = NaN;
        [~, col] = find(isnan(groupTLists{15}.f2(:,1:floor((length(alignedS2D))/2))));
        groupTLists{15}.f2(:,1:max(col)) = NaN;
        
        % find lowest nan in second half
        [~, col] = find(isnan(groupTLists{15}.f1(:,ceil((length(alignedS2D))/2):end)));
        groupTLists{15}.f1(:,min(col)+ceil((length(alignedS2D))/2)-1:end) = NaN;
        [~, col] = find(isnan(groupTLists{15}.f2(:,ceil((length(alignedS2D))/2):end)));
        groupTLists{15}.f2(:,min(col)+ceil((length(alignedS2D))/2)-1:end) = NaN;
    end
    
    %% Get group average sim stimulus change to button press delays
    groupl2hDelay = nanmean(l2hAvgDelays);
    grouph2lDelay = nanmean(h2lAvgDelays);
    groupDelay = nanmean(avgDelays);
    ...
    for tType = [13:15]; %[11:12 15] %11:12
        ...
        figure
        title([analGroupIDs{iGroup} ' ' tLabels{tType} ': ' num2str(size(groupTLists{tType}.f1, 1)) ' participants averaged'], 'FontSize', 16)
        hold on
        if tType == 15 && alignToCross
        ...
        if ~useSimSchedule && tType == 14 % sim plot
            vline(-groupDelay, 'b', 'stim change')
        ...
        elseif alignToCross && tType == 15 % normalized rivalry plot
            xlabel('Time from crossing of RLS traces');
        else
        ...
        if allTrans
            figName = [groupTransPlotDir analGroupIDs{iGroup} '_' tFileNames{tType} '_' num2str(size(groupTLists{tType}.f1,1)) 'pars_ALL.jpg'];
        else
            figName = [groupTransPlotDir analGroupIDs{iGroup} '_' tFileNames{tType} '_' num2str(size(groupTLists{tType}.f1,1)) 'pars.jpg'];
        end
