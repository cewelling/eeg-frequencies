...
clearvars -except pValues kSize cutOff rValues ampClust groupNum classifyPercept crossVal airplane diagnose
...
for iGroup = 1:length(groupParNums)
    for parNum = groupParNums{iGroup}
        ...
        if exist(['transitions/finalTransitions_norm/' parName '_' normType '_' analFreqLabel '_' electrodeSet.name '.mat'], 'file')
...
for i = [5] %[2 4:5] %size(fullAnalMatrix, 2)
    for j = [17]; %[12:17] %i+1:size(fullAnalMatrix,2)
        ...
        % save figures
        if strcmp(analType, 'darts_dartRival')
            figName = [corrPlotDir C{i} 'vs' C{j} '_DARTS'];
        else
            figName = [corrPlotDir '_' C{i} 'vs' C{j} '_MARZ'];
        end
...
for i = [3];%[5] %[2 4:5] %size(fullAnalMatrix, 2)
    for j = [19:20]; %[17]; %[12:17] %i+1:size(fullAnalMatrix,2)
        for iGroup = 1:length(groupParNums)
            figure
            hold on
            %if abs(P(i,j)) < 0.1
            ...
            % save figure
            if strcmp(analType, 'darts_dartRival')
                figName = [corrPlotDir C{i} 'vs' C{j} '_DARTS_' analGroupIDs{iGroup} ];
            else
                figName = [corrPlotDir '_' C{i} 'vs' C{j} '_MARZ_' analGroupIDs{iGroup}];
            end
            saveas(gcf,figName,'jpg')
            saveas(gcf,figName,'pdf')
        end
...
for iIndex = [17]; %[1 5 16 17 18];%[17]; %[1 5 17] % 12:17
    ...
    text(1.5, yax(2) - (yax(2)-yax(1))*0.1,['p = ' num2str(P)],'FontSize', 12)
     ...
     % save bar graphs
        if strcmp(analType, 'darts_dartRival')
            figName = [barPlotDir C{iIndex} '_DARTS'];
        else
            figName = [barPlotDir '_' C{iIndex} '_MARZ'];
        end
        saveas(gcf,figName,'jpg')
        saveas(gcf,figName,'pdf')
end
